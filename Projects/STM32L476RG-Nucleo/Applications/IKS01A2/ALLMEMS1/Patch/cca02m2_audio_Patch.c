    ...
    else if(Instance == 1U)
    {
      ...
      for(i = 0; i < DFSDM_MIC_NUMBER; i ++)
      {
        ...
        if(((AudioInit->Device >> (uint8_t)i) & AUDIO_IN_DIGITAL_MIC1) == AUDIO_IN_DIGITAL_MIC1)
        {
          ...
          if(CCA02M2_DFSDM1_Init(&hAudioInDfsdmFilter[i], &hAudioInDfsdmChannel[i], &dfsdm_config) != HAL_OK)
          {
...
* @retval HAL_status
*/
__weak HAL_StatusTypeDef CCA02M2_DFSDM1_Init(DFSDM_Filter_HandleTypeDef *hDfsdmFilter, DFSDM_Channel_HandleTypeDef *hDfsdmChannel, MX_DFSDM_Config *MXConfig)
{  
  HAL_StatusTypeDef ret = HAL_OK;
